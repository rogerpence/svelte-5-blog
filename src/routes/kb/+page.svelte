<script lang="ts">
	...
	import { getPostsPage } from '$lib/remote-funcs/get-posts.remote';
	...
	async function loadData(pageNum: number): Promise<void> {
		try {
			error = null;
			data = await getPostsPage({ pageNumber: pageNum });
		} catch (err) {
	...
	const folder = page.url.pathname.split('/')[1];
	const navRoute = `/${folder}?pagenumber=`;
